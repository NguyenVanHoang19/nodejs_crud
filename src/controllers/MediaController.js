const { Controller } = require("../../systems/controllers/Controller");
...
const fs = require("fs");
const utils = require("../../systems/helpers/Utility"),
...
class MediaController extends Controller {
  ...
  async get(req, res, next) {
    ...
    try {
      ...
      const filePath = `${req.protocol + '://' + req.get('host')}/uploads/${response.data.filename}`;
      return res
        .status(response.statusCode)
        .json({ ...response, data: { ...response.data, filePath } });
    } catch (e) {
      next(e)
    }
  }
